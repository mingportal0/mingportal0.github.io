          ...
          if (action.type === 'link' || action.type === 'linkExternal') {
            element = (
              <Link key={index} href={action.link ? action.link : '/'} passHref>
                <a
                  data-tinafield={`${parentField}.${index}`}
                  className={`group inline-flex items-center text-lg font-semibold text-white transition duration-150 ease-out hover:text-gray-50`}
                  style={{
                ...
                >
                  ...
                  {action.icon && (
                    <BiRightArrowAlt
                      className={`ml-0 mr-0 h-6 w-6 opacity-80`}
                    />
                  )}
                </a>
              </Link>
